...
# 02 Sep 26 Windows port - DUMPDIR, PRE_RELEASE_FIXES.md 28.  Set, not left
# empty: pdump.c:98 falls back to sysseg->sysdir when this is blank, and that
# is the system directory, which grants sdusers Modify - so a process dump,
# which carries the whole variable state of the session that wrote it, was
# readable by every SD user.  secure-dumps.ps1 makes this directory write-only
# to sdusers, so a dump can be added and nobody else's can be listed or read.
