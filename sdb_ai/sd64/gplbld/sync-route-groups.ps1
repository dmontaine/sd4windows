...
try {
    ...
    # -----------------------------------------------------------------------
    # 18 Sep 26 - ***THE ADMINISTRATOR RULE, ENFORCED ON EVERY INSTALL.***
    # RELEASE_1.1_FIXES.md 62 and 63.  Owner, 18 Sep 2026: an administrator has
    # NO ssh from anywhere, and the API from this machine only.
    #
    # ***WHY IT HAS TO BE HERE AND NOT ONLY IN THE VERBS.***  58 and 62 changed
    # what CREATE.ACCOUNT and MODIFY.ACCOUNT grant FROM NOW ON, and migrated
    # nothing - and sdssh/sdapi are WINDOWS LOCAL GROUPS, so they and their
    # membership survive a cycle: deleting both SD trees does not touch them.
    # Measured on b197, on the owner's own account: verify-accountrules scored
    # "an ADOPTed account has NEITHER route: expected none, got ssh+api", and
    # ace\Don is a Windows administrator that held both on a tree the new build
    # had just installed.  A grant made before 58 had never been withdrawn.
    # That is PRE_RELEASE 70's warning - an upgrade replaces files and re-runs
    # nothing - so the migration belongs in a step the installer always runs.
    #
    # THIS SCRIPT IS THAT STEP: sd.iss calls it on EVERY install including an
    # upgrade, deliberately ungated (see its SyncRouteGroups comment), and it
    # already owns both groups.
    #
    # ***AND IT INVERTS A DOCUMENTED PROPERTY OF THIS SCRIPT, ON PURPOSE.***
    # sd.iss's justification for leaving it ungated was that it "seeds only the
    # group IT created, so on a tree that already has sdssh it declines to seed
    # and changes nobody's access".  The seeding half still works that way; this
    # half DOES change access on an existing tree, and must, or the rule never
    # reaches the accounts that predate it.  That comment is corrected in the
    # same commit.
    #
    # ONLY SD's OWN ACCOUNTS ARE TOUCHED.  The withdrawal walks sdssh, whose
    # membership is SD's by construction; the grant walks sdusers.  Nothing
    # reaches into Administrators itself, and no principal that is not already
    # in one of SD's groups is looked at - so a domain group or the built-in
    # Administrator account is left exactly as it was.
    $admins = $null
    try {
        $adminGrp = Get-LocalGroup -SID 'S-1-5-32-544' -ErrorAction Stop
        $admins   = @(Get-LocalGroupMember -Group $adminGrp.Name -ErrorAction Stop |
                      ForEach-Object { ($_.Name -split '\\')[-1] })
    } catch {
        # A lookup that FAILED is not "nobody is an administrator".  Saying so
        # and failing is the only honest answer: acting on an empty list would
        # silently leave every administrator holding ssh.
        Say "COULD NOT READ the administrators group - $($_.Exception.Message)"
        Say "the administrator route rule (RELEASE_1.1 62) was NOT applied; nothing was changed"
        $failed = $true
    }

    if ($null -ne $admins) {
        Say ("administrators seen: " + $(if ($admins.Count) { $admins -join ', ' } else { '<none>' }))

        # (1) WITHDRAW ssh.  Walk sdssh and take out anyone who is an
        # administrator.  Idempotent: an administrator who is not in it is not
        # an error and is not reported as a change.
        $sshNow = Get-Members $SshGroup
        if ($null -eq $sshNow) {
            Say "could not read $SshGroup - the ssh withdrawal was not applied"
            $failed = $true
        } else {
            foreach ($m in $sshNow) {
                $short = ($m -split '\\')[-1]
                if ($admins -notcontains $short) { continue }
                if ($Check) { Say "would REMOVE $m from $SshGroup (administrator, RELEASE_1.1 62)"; continue }
                try {
                    Remove-LocalGroupMember -Group $SshGroup -Member $m -ErrorAction Stop
                    Say "REMOVED $m from $SshGroup - an administrator gets no ssh (RELEASE_1.1 62)"
                } catch {
                    Say "FAILED to remove $m from $SshGroup - $($_.Exception.Message)"
                    $failed = $true
                }
            }
        }

        # (2) ENSURE the API, for SD's own administrator accounts.  62 makes the
        # API a RULE for the tier rather than a setting, so an administrator
        # that predates it must GAIN it - the mirror of (1), and the half that
        # keeps an administrative GUI application working after an upgrade.
        $sdNow = Get-Members $SeedFrom
        if ($null -eq $sdNow) {
            Say "could not read $SeedFrom - the API grant was not applied"
            $failed = $true
        } else {
            foreach ($m in $sdNow) {
                $short = ($m -split '\\')[-1]
                if ($admins -notcontains $short) { continue }
                if ($Check) { Say "would ADD $m to $ApiGroup (administrator, RELEASE_1.1 62)"; continue }
                try {
                    Add-LocalGroupMember -Group $ApiGroup -Member $m -ErrorAction Stop
                    Say "added $m to $ApiGroup - an administrator has the API from this machine (RELEASE_1.1 62)"
                } catch {
                    if ($_.Exception.Message -match 'already a member') {
                        Say "$m is already in $ApiGroup"
                    } else {
                        Say "FAILED to add $m to $ApiGroup - $($_.Exception.Message)"
                        $failed = $true
                    }
                }
            }
