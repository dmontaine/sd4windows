...
***NEXT FREE ID: 63.*** Take it from here and increment it; **do not derive it by
...
| 62 | **B?** | ***RE-MEASURE WHETHER `elevate('START')` STILL ADMITS SOMEBODY WHO IS NOT AN ADMINISTRATOR.*** Split out of **56** when that closed, 29 Aug 2026, so a measured finding is not lost because the code around it moved. **As measured before clause 2 was reversed**: `elevate('START')` gates on `Start-Process -Verb RunAs` succeeding (`gplbld/sd-elevate.ps1:120`) and **tests nobody's identity** — that verb gives a standard user a *credential* prompt, so an administrator's password was enough to reach SDSYS, and `@logname` and `audit_message()` then named the standard user who did **not** consent. ***THE REVERSAL PLAUSIBLY CLOSES IT AND THAT IS NOT MEASURED***: `LOGIN:568` now requires the session to be **already elevated**, and such a session runs as the administrator whose credentials were typed, so the trail should name the right person — but `logto sdsys` is a second route with its own gate (`CPROC`), and neither has been re-measured since. **Sev is `B?` until it is**: if it survives it is a `B`, and if it does not this entry closes with the measurement written down | `gplbld/sd-elevate.ps1:120`, `gpl.bp/ELEVATE`, `gpl.bp/CPROC` |
