...
> | | |
> |---|---|
> | install | **16 Sep 22:55 — the relay build's THIRD cycle**; the full suite (`b173`) and `b174` ran on it. ***STALE SINCE 17 Sep ~01:30: `gplsrc/sdwind.c` CHANGED (RELEASE_1.1 37's fix) AND `sdsys/changelog` — A CYCLE IS OWED*** |
...
>
> **NEXT — ONE CYCLE, THEN THE 37 WITNESS.** Both **elevated PowerShell**. The cycle rebuilds `sdwind.exe` (step 0) and installs:
> ```powershell
> powershell -ExecutionPolicy Bypass -File C:\Users\Don\SDCoreProject\sd4windows\sdb_ai\sd64\gplbld\cycle.ps1
> ```
> Then `verify-deadlock` (in neither runner; it kills one SD session it created and waits up to six minutes for the daemon's tick — expect `named` for up to five minutes, then `none`, exit 0):
> ```powershell
> powershell -ExecutionPolicy Bypass -File C:\Users\Don\SDCoreProject\sd4windows\sdb_ai\sd64\gplbld\verify-deadlock.ps1
> ```
> *Falsified-if: the state stays `named` past six minutes (the daemon still did not reap — read `errlog` for `Lost user`/`Cleanup did not complete`), or `errlog` lacks the daemon's `Lost user … running … -cleanup` line while the lock cleared (something else reaped it). A killed session is a staged fault: `assert-current` is unaffected, but run the verify BEFORE any suite step you want to trust, and a cycle is expected after it as its own last line says.*
>
