...
> | | |
> |---|---|
> | install | **16 Sep 11:53:16**, unchanged — and ***THE TREE IS NOW STALE BY DESIGN***: product source changed this session (`sd_tlssrv.c`, `win32relay.c` new, `win32s4u.c`, `sd_tls.h`, `gpl.src`, `Makefile`, `stage.py`, `install-service.ps1`, `sdtlsrelay/`, `changelog`). `bin\sd.exe` is still the 15 Sep one; `bin\sdtlsrelay.exe` was built by `make sdtlsrelay` for the unit test |
> | tokens | `b167` **still unspent** |
> | git | all committed and pushed on `origin/main` |
> | free tier | **42 green + `test-sysmsg-units` exit 2** (this shell predates the 11:53 install, PRE_RELEASE 182 — the documented non-fault). **Forty-three now**: `test-tlsrelay-units.py` joined |
...
> 1. **The cycle** (elevated PowerShell). Step 0 rebuilds the C including the new `sdtlsrelay` target; `stage.py` will print `sdtlsrelay.exe is native (N imports, none msys-*)` or refuse. The install runs `install-service.ps1 -Install` hidden, and its output goes nowhere — so **`Get-LocalUser sdrelay` is the first thing to look at after the cycle**; `verify-relayidentity`'s step 1 checks it exists, is enabled and is in no group, and refuses everything else if it is absent.
> ```powershell
...
> ```powershell
> powershell -ExecutionPolicy Bypass -File C:\Users\Don\SDCoreProject\sd4windows\sdb_ai\sd64\gplbld\VerifyInstall2.ps1 -Run b167 -Only verify-relayidentity,verify-apiport,verify-scramlogin,verify-apiidentity
> ```
