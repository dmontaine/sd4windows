...
places that carried it. The thirty-eighth: **§2, §4, §7 and §8 compressed, and
nothing else** — two markdown files at the repository root, which
`assert-current` does not walk and would exclude anyway
(`assert-current.ps1:372` drops unshipped `.md`). **The 17:18:11 install is
still current — re-measured, exit 0** — and the suite below still stands.
**Start from it.**
...
`## ARCHIVE 21 Aug 2026` entries: the LocalSystem exposure record, §7's closed
steps 0–2 and 4–6, the phase-by-phase header, §4's measurement record, §8's
closed questions, §2's generation-2 audits and dev-branch review, and §7's
closed steps 7, 8 and 11. **§2, §4, §7 and §8 are at their conclusions.**

**THERE IS NO CHEAP MATERIAL LEFT, so stop looking for a next target.** What
remains is §5, the reasoning that dates, and §6, which rule 4 puts off limits —
and the two together were always larger than the overshoot, so **the ceiling
was never reachable by compressing closed work.** Treat 3,500 as a direction and
not a number to hit; if it has to be met, that is a decision to shorten §5, and
the owner's, not a tidying job. The line counts that used to stand in this
...
|---|---|---|
| §7 step 6 | `APIPORT` defaults off, transport is posture B | both reversed by Phase 1 |
| §4 | `OS.EXECUTE` on an installed system is unknown | answered 15 Aug, in §4's own verified half |
| §4, §7 step 7, §8 | gating `OS.EXECUTE` "needs C" and `OS.EX` is read by nobody | **that C landed 19 Aug** — `os_permitted()`, `op_sh.c:150` |
| §8 | *"BUILT AND VERIFIED: `RDPACCOUNT`"*, 100 lines | the keyword was **deleted the next day** |
| §2 | `SV_EMSG_PAIR`/`SV_ECONTXT` still transposed | fixed 15 Aug; UPSTREAM_FIXES #2 **closed**; all four trees read 6/7 |
| §2 | `sdsys/syscom/sdclilib.h` defines neither | it defines both, `:127` |
