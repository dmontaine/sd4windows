...
> | ***owed, in order*** | 1. ***DONE — cycle green; `dll_closure` copied OpenSSL beside `sd.exe`, measured.*** 2. ***FUNCTIONAL WITNESS DONE*** — `verify-apiport.ps1` (elevated, 15 Sep) all checks: the client DLL carried a session through TLS, SCRAM 47/48, no cleartext, wrong pw + SDSYS refused. **Still owed:** a packet capture on 4243 (proves ciphertext; the test reads the client's logical stream, not the wire), a plaintext-refused control (comes with 42), and a Windows↔Linux interop run (blocked on a Linux test account, owner asked; this box is `192.168.0.2`, Linux `192.168.0.210:4243`). 3. ***RELEASE_1.1 42 — STARTED*** — the plaintext API verifiers (`verify-scramlogin`, `verify-peerlog`, `verify-apiidentity`'s raw leg) break against the TLS-only server; plan (from Linux parity): positives via the client DLL, raw socket kept as a `--no-tls`/`--no-binding` refusal control. 4. uninstaller removal of `sd-tls` (finding 38) |
> ```powershell
