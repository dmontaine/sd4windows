...
> | ***owed, in order*** | 1. ***DONE — cycle green; `dll_closure` copied OpenSSL beside `sd.exe`, measured.*** 2. ***FUNCTIONAL WITNESS DONE*** — `verify-apiport.ps1` (elevated, 15 Sep) all checks: the client DLL carried a session through TLS, SCRAM 47/48, no cleartext, wrong pw + SDSYS refused. **Still owed:** a packet capture on 4243 (proves ciphertext; the test reads the client's logical stream, not the wire), a plaintext-refused control (comes with 42), and a Windows↔Linux interop run (blocked on a Linux test account, owner asked; this box is `192.168.0.2`, Linux `192.168.0.210:4243`). 3. ***RELEASE_1.1 42 — FOUNDATION DONE, VERIFIER REWIRING NEXT.*** Built and unit-green (no server): `gplbld/scram-probe.py` (Windows port of Linux's TLS+SCRAM probe — libssl-3-x64.dll via ctypes; RFC 7677 vector matches) + `test-scramprobe-units.py` 9/9 (free tier 41). **Still to do in 42:** rewire `verify-scramlogin.ps1`'s positive + refusal rows onto `scram-probe.py` (`--no-tls`/`--no-binding`/`--final-only`/`--legacy` are its controls), keep its raw `TcpClient` only as the plaintext-refused row; then `verify-peerlog.ps1` and `verify-apiidentity.ps1`'s raw leg. **Those need an elevated run to witness** (they set up APIPORT + a throwaway account like `verify-apiport`). The probe usage mirrors `verify-apiport`'s account setup: `SD_SCRAM_PASSWORD=<pw> python gplbld/scram-probe.py --user <acct> --account <ACCT> [--final-only|--no-binding|--legacy|--no-tls]`. 4. the live-wire capture on 4243 + Windows↔Linux interop (blocked on test accounts, owner asked Linux). 5. uninstaller removal of `sd-tls` (finding 38) |
> ```powershell
