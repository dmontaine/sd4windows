...
>
> **Owed:** `verify-deadlock` run 3, **elevated**, no cycle first (`verify-deadlock.ps1` is in `$neverShipped`, so the install stays current). A cycle after it **only if its last line says a session was killed**. Then 5's remaining half (the real W1.0-0 upgrade, owner: later) and 18 (at W1.1-0 assembly).
> ```powershell
> powershell -ExecutionPolicy Bypass -File C:\Users\Don\SDCoreProject\sd4windows\sdb_ai\sd64\gplbld\verify-deadlock.ps1
> ```
> *Falsified-if*: the instrument row must now read `state named` with `LOCK user=<n> type=RU name=[SDSYS]`; a third `seen=0` would mean the miss is not the case fold (the `OTHER-LOCK` line will show the id GETLOCKS actually holds). After the kill, exit 2 "not reached" remains as likely as 0 (Handoff 69's note stands); a refusal at "exactly one new sd.exe" would mean another session started meanwhile.
>
