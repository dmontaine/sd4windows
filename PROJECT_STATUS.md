...
> | tokens | **`b167`–`b171` SPENT** (`b170` 23:09 died on a pre-cycle token, PRE_RELEASE 182 — the owner signed out and back in; `b171` 23:16 below). **Use `b172`** |
...
> ```powershell
> powershell -ExecutionPolicy Bypass -File C:\Users\Don\SDCoreProject\sd4windows\sdb_ai\sd64\gplbld\VerifyInstall1.ps1 -ThenElevated -Run b172
> ```
> *`verify-lcnames` is step 12 of the first half; if it is green there the re-entry worked live, and the run carries on into the second half where the relay steps are.*
