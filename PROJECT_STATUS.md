...
> | tokens | **`b167`–`b176` SPENT** (`b170` 23:09 died on a pre-cycle token, PRE_RELEASE 182 — the owner signed out and back in; `b175` and `b176` were `-Only verify-print`). **Use `b177`** |
...
>
> | ***54 WITNESSED ON THE 02:30 CYCLE — THREE WAYS.*** | **(1) `verify-print b176`, named leg**: `SETPTR 0,80,60,0,0,1,AT sdprnb176a,BRIEF` + `LIST … LPTR`, no refusal, the listing read back from the file-port printer's port file (`WHO` and the run marker) — the product path `linuxprt.c` → `powershell Out-Printer -Name` → spooler, end to end on the install. **(2)(3) owner, interactive session, no `AT`**: *"printed correctly to pdf and physical printer"* — the default-printer path, to `Microsoft Print to PDF` and then to a real printer he attached. ***THE VERIFIER'S OWN DEFAULT LEG IS NOT YET GREEN AND BOTH REDS WERE THE HARNESS***: `b175` refused (`Get-PrinterDriver` lists INSTALLED drivers; `Generic / Text Only` ships STAGED in `prnge001.inf` — now `Add-PrinterDriver` from the store, removed again at the end, `2fd9142`); `b176` scored the default leg red because `LegacyDefaultPrinterMode = 0` ("Let Windows manage my default printer") **ignores `SetDefaultPrinter`**, so SD printed to the PDF default exactly as told — now the mode is set 1 for the run and restored, and the run **refuses, exit 2**, if the default did not take rather than scoring a leg it does not control (`859c0e9`). `b177` is owed for a green verifier; **the product fact does not wait on it.** Docs (48): SETPTR's BANNER/LANDSCAPE ignored, `SENDMAIL` unavailable |
