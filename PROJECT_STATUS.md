...
>
> **NEXT, in order.**
> 1. ***Probe Cygwin `SCM_RIGHTS` fd-passing*** — the next socket-handover mechanism. A Cygwin process passes a socket fd to another Cygwin process (different token) over an AF_UNIX socket via `sendmsg`/`SCM_RIGHTS`, which rebuilds a real socket fhandler in the receiver — unlike the raw-handle adopt that just failed with EINVAL. If it crosses the token boundary, the daemon accepts and passes the fd to the bare relay. Extend `probe-relaydrop` (reuse its S4U mint + `CreateProcessAsUser` scaffolding) or start `probe-relaysock`. First confirm Cygwin/MSYS2 even implements `SCM_RIGHTS` for sockets on this build.
> 2. ***If `SCM_RIGHTS` fails too, architecture B*** — the bare relay owns the listener and `accept()`s its own connections (its sockets are its own Cygwin sockets → no cross-token network-socket handover). The remaining handover is relay→session plaintext, brokered by the LocalSystem daemon.
