...
>
> **IN FLIGHT — 5's remaining half, the real upgrade (owner: build then run tonight, 15 Sep).** Version bumped to **W1.1-0** in source (sd.iss `AppVer`; the release string in gplsrc/revstamp.h, gplsrc/sdclilib/revstamp.h, sdsys/gpl.bp/revstamp.h, and both `$release` records — segment numerics left as W1.0-0 shipped them). Harness `verify-realupgrade.ps1` built (`-Snapshot`/`-Compare`), parse 0/5, probe bbcmp rc 0, free tier 39/39, revstamp consumers syntax rc 0. **Nothing cycled or run.** The destructive run is owed, in this order (all elevated) — a `cycle.ps1 -SkipInstall` first to build `sd-setup-W1.1-0.exe` (the SD_REV_STAMP change means step 0 recompiles the C):
> ```powershell
> powershell -ExecutionPolicy Bypass -File C:\Users\Don\SDCoreProject\sd4windows\sdb_ai\sd64\gplbld\cycle.ps1 -SkipInstall
> ```
> Then install shipped **W1.0-0** (from `SDCore-W1.0-0.zip`, the pre-D2 tree — NOT the sdout copy, which is the current build), `-Snapshot`, install the new **W1.1-0** over the top, `-Compare`, then a plain `cycle.ps1` to restore a clean current install. Exact staged paths are in the session hand-over.
> ***TRAP, found 15 Sep by the owner: installing W1.0-0 over an existing tree is a `DataTreeUpgrade`, which PRESERVES per-account data — so the accounts (and the per-account VOCs, SDSYS's included) stay in their 1.1 NOCASE state and the tree is a W1.0-0/1.1 mix, not pre-D2.*** The W1.0-0 install MUST be onto an ABSENT data tree (`DataTreeAbsent`), so the pre-step is: `sc stop SD`, uninstall (`C:\Program Files\SD\unins000.exe /VERYSILENT`), then delete `C:\ProgramData\SD` outright — only that clears the 1.1 account data. Then W1.0-0 installs fresh and re-adopts `Don` (from the live Windows user) as a pre-D2 case-sensitive account. `verify-realupgrade -Snapshot`'s precondition (fixture + SDSYS VOC `FL$NOCASE=0`) is what proves the tree really is pre-D2 before the run spends anything.
> **Also open:** **37** (owner's call on severity; narrowing it means staging a dead lock again via `verify-deadlock`, then `sd -cleanup` elevated + `LIST.READU`: lock gone → detection `kill(pid,0)` at `sdwind.c:262`; lock stays → `sd -cleanup` itself); **7** (unwitnessed, route depends on 37); **18** (at W1.1-0 assembly). *Item 7 may have no route to a witness by killing while 37 holds*: on this one run the owner was not unmapped while its lock stood, and `(gone)` needs the opposite order (slot gone, lock still there). One run, one window; do not treat it as the rule until a second run or the 37 probe says so.
> ```powershell
