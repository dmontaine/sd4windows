...
> # ⇩⇩⇩ HANDOFF 82, 17 Sep 2026 — ***RELEASE_1.1 55 IS THE BLOCKER, AND IT IS THE ONE THE PORT HAS CIRCLED SINCE AUGUST: EVERY AUTHENTICATED API SESSION IS STILL A LocalSystem PROCESS. 43 CARRIED THE FIX THROUGH FOR THE RELAY ONLY. THE TWO CLEAN ROUTES FOR THE SESSION WERE EACH CLOSED OFF BY A CONSTRAINT THAT HAS SINCE CHANGED. NOTHING IS OWED ON THE TREE; TOMORROW STARTS WITH A DECISION AND A PROBE, NOT A CYCLE.*** ⇩⇩⇩
>
> | | |
> |---|---|
...
> | ***THE TWO DIRECTIONS, CONDITIONAL*** | **(a) broker + spawn-as-user** — the token side is proven; its unproven crux is that the SD session is MSYS2/Cygwin and 43 measured a spawned Cygwin child inherits **neither socket (`EINVAL`) nor pipe (`EBADF`)** — the same wall that forced the relay native — so handing the session its connection is the real unsolved problem; and it keeps the SCRAM parse privileged unless the check moves to the unprivileged front (the OpenSSH/RDP-NLA shape). **(b) `LogonUser` over TLS** — off its stale ban now that 41 exists; needs no `SeTcb`, sidesteps BOTH the 1314 wall and the socket handover, and lets the service move to a service account. *Costs*: the server sees the password during login (inside TLS); the API login Linux also speaks changes (a **parity decision — coordinate by mail**); it checks the WINDOWS password while the API checks SD's own (PRE_RELEASE 42); and the clients do NOT verify the server certificate today (`SSL_VERIFY_NONE`, `sd_tls.c:237`) — safe only while no password is sent, so (b) also needs a way for clients to recognise the real server |
...
> | ***NEXT — A PROBE BEFORE ANY PRODUCT CODE, THEN THE OWNER DECIDES (a) vs (b)*** | Build a probe that, running as a **non-SYSTEM service account**, calls `LogonUser` with a test password and `CreateProcessAsUser`, and confirms the child is the user with `SeTcb` **absent** — i.e. that (b) needs no SYSTEM. *Falsified-if:* it refuses without `SeTcb` (then (b) collapses toward (a)). If it holds, (b) is the smaller, cleaner route. **53** (the writable MSYS2 `shared.5`) is decided with 55 — it is only dangerous while a SYSTEM process uses SD's MSYS2 runtime, which 55's fix removes. The measured facts here are the History, Linux and Prior-art rows; everything about the fix is conditional until the probe |
>
