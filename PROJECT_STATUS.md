...
> | | |
> |---|---|
> | install | **17 Sep 02:30:50, the 54 cycle** (`bin\` built 02:29:40; `assert-current` green `b175`–`b178`). ***STALE SINCE ~03:05: `linuxprt.c` AND `changelog` CHANGED (the PowerShell-dump fix, owner's "fix powershell dump") — A CYCLE IS OWED*** |
> | tokens | **`b167`–`b178` SPENT** (`b170` 23:09 died on a pre-cycle token, PRE_RELEASE 182 — the owner signed out and back in; `b175`–`b178` were `-Only verify-print`, the last 13/13). **Use `b179`** |
...
>
> **NEXT — ONE CYCLE, THEN `verify-print` ONCE MORE (15 checks now), THEN THE LAST TWO ROWS BEFORE 47.** Owner, 17 Sep ~03:00: *"fix powershell dump"* — a wrong `AT` name showed the user PowerShell's six-line error dump (`At line:1 char:`, carets, `CategoryInfo`) before SD's one line. `linuxprt.c`'s `-Command` text now wraps the pipeline in `try { } catch { [Console]::Error.WriteLine($_.Exception.Message); exit 1 }`, leaving Windows' one sentence and SD's; driven by hand against a bogus printer and a missing job file (one stderr line, exit 1, both); `-fsyntax-only` clean. `verify-print`'s refusal leg gained two checks: Windows' sentence present, and none of `CategoryInfo|FullyQualifiedErrorId|At line:\d+ char:` — **the new checks would go RED on the 02:30 install, which is the point.** Both **elevated PowerShell**:
> ```powershell
> powershell -ExecutionPolicy Bypass -File C:\Users\Don\SDCoreProject\sd4windows\sdb_ai\sd64\gplbld\cycle.ps1
> ```
> ```powershell
> powershell -ExecutionPolicy Bypass -File C:\Users\Don\SDCoreProject\sd4windows\sdb_ai\sd64\gplbld\VerifyInstall2.ps1 -Run b179 -Only verify-print
> ```
> *Falsified-if: the refusal leg's session text still carries `CategoryInfo`, or Windows' sentence is missing (the catch swallowed it), or either of the two print legs regresses — the `try` wraps them too.* *(If `verify-print` ever dies mid-run: `Get-Printer` shows `sdprn<Run>a`/`sdprn<Run>d` to remove, the user's default printer to reset, and `LegacyDefaultPrinterMode` under `HKCU\Software\Microsoft\Windows NT\CurrentVersion\Windows` to put back — the owner's box reads 1, his own choice.)* Then **40** (a real W1.0-0 → W1.1-0 upgrade to witness the captions — the owner's hand) and **53** (a ruling — accept as the runtime's known limitation was the recommendation), then 47 → 48 → 49. **48 must say**: `SETPTR` BANNER/LANDSCAPE and the form name are ignored, `SENDMAIL` is unavailable, printing goes to the Windows default or the `AT <printer name>` printer, **and "Let Windows manage my default printer" (on by default in Windows 10+) makes that default THE LAST PRINTER USED FROM ANY PROGRAM** — the owner had to turn it off and pick his printer before a no-`AT` job went where he meant (17 Sep); a user with several printers should do the same or name the printer with `AT`. Owner, 17 Sep: no printer-listing verb in W1.1 — the name comes from Windows Settings, as upstream; **`LIST.PRINTERS` is deferred to W1.2** (RELEASE_1.1_FIXES.md `## Deferred to W1.2`, D1 — the section's first entry). `Microsoft Print to PDF` sits on `PORTPROMPT:` (a Save dialog): fine at a console, no desktop to show it from SSH or the API.
>
