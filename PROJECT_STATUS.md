...
> ***⏸ PICK UP HERE (17 Sep 2026).*** Tree **clean**. **Install current** (17 Sep 04:00:54); **`b181` unspent** — probes + compile-checked C only, **no cycle**. **RELEASE_1.1 55 is fully de-risked** (5 probes; every mechanical unknown green) and the **parity product build is under way**: slices **1–3 done, compile-clean**. 1–2: `win32_session_spawn` spawns `sd -N -H` as the user with the pipe on std handles; `-H`/`api_preauth` makes the session skip relay/ACK/SCRAM/peer-ID. 3: the front↔relay **control channel** — a second socketpair, protocol at the end of `gplsrc/sd_tls.h`, `sd_tls_relay_pipe()` in `sd_tlssrv.c`, relay argv now `<net> <sp> <ctl> <ms>`; `test-tlsrelay-units.py` **33/33** with two mutants. ***NEXT = SLICE 4***: the relay becomes the pipe SERVER and cuts over (`probe-relaycutover` is the measured shape), then 5 (`op_kernel` `K$HANDOFF`), 6 (APISRVR skip-SCRAM — BASIC, needs the cycle). Full file-by-file spec + the **two corrections caught 17 Sep** (the `select()` always-ready trap → use std-handles not `cygwin_attach_handle_to_fd`; Linux parses SCRAM as root → crux 2 dropped) are in **HISTORY.md 17 Sep** and the rows below.
>
> ⚠️ ***SLICE 4's ONE UNMEASURED STEP, AND IT IS NOT IN ANY PROBE: CAN THE RELAY — `sdrelay`, Low integrity, ZERO privileges — `CreateNamedPipe` AT ALL?*** `probe-relaycutover` ran the relay as the ordinary caller at **Medium**, so it says nothing about this. It is written to **fail closed and loud** (`SD_RELAY_CTL_FAILED` with the reason, front refuses the login) rather than hang, but if it turns out Low cannot create the pipe the topology changes: the FRONT would create it and hand the relay the server handle at spawn. **Do not assume either way from the probes.**
>
