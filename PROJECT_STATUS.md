...
> | | |
> |---|---|
> | install | **17 Sep 01:25:28, re-cycled after the reboot** (`assert-current` green from this shell, `sdwind` up 01:43:20). ***STALE SINCE ~02:10: `linuxprt.c`, `lnx.c`, `op_sh.c`, `sd.h`, `changelog` CHANGED (RELEASE_1.1 54) — A CYCLE IS OWED*** |
> | tokens | **`b167`–`b172` SPENT** (`b170` 23:09 died on a pre-cycle token, PRE_RELEASE 182 — the owner signed out and back in; `b171` 23:16 and `b172` 23:39 below). **Use `b173`** |
...
>
> | ***54 RULED AND BUILT, NOT CYCLED: SETPTR MODE 1 PRINTS TO A WINDOWS PRINTER (`Out-Printer`, the default or `AT`'s), `SENDMAIL` REFUSES WITH `ER_UNSUPPORTED`*** | Owner: *"the default windows printer is what upstream open qm does … sendmail not being available is fine."* `linuxprt.c` `spool_print_job()` `fork`/`execl`s PowerShell (found by `sd_powershell_path()`, lifted from `op_sh.c`) and waits; refusals are said on the terminal. `lnx.c` is one refusal. Compiles clean. **The witness is `verify-print.ps1`** (VerifyInstall2 after `createfilecase`, `sdprn<Run>`, declared `$notProfiles`): two `Generic / Text Only` printers on FILE-PATH LOCAL PORTS, one made the user's default for the run, job text read back; named, default and refusal legs. ***THE HARNESS ITSELF IS UNTRIED*** — `Add-PrinterPort` on a path and that driver have not been exercised on this box (the real default is `Microsoft Print to PDF` on `PORTPROMPT:`, a dialog). Read its first run as a measurement of both. The reboot cleared 35 of 36 orphaned profiles; the 36th's stem was added (`fd02000`) |
>
> **NEXT — ONE CYCLE, THEN THE PRINT WITNESS.** Both **elevated PowerShell**:
> ```powershell
> powershell -ExecutionPolicy Bypass -File C:\Users\Don\SDCoreProject\sd4windows\sdb_ai\sd64\gplbld\cycle.ps1
> ```
> ```powershell
> powershell -ExecutionPolicy Bypass -File C:\Users\Don\SDCoreProject\sd4windows\sdb_ai\sd64\gplbld\VerifyInstall2.ps1 -Run b175 -Only verify-print
> ```
> *Falsified-if: a port file gains no text (either the spooler did not write a file-path port, or `Out-Printer` did not run — the session's own `Print job not sent` line tells which), or the refusal leg puts a job in a file. It changes the user's default printer for the run and puts it back; if it dies mid-run, `Get-Printer` will show `sdprnb175a`/`sdprnb175d` to remove by hand and the default to reset.* Then the rows before 47: **40** (a real W1.0-0 → W1.1-0 upgrade to witness the captions), **53** (a ruling — accept as the runtime's known limitation was the recommendation).
>
