...
> # ⇩⇩⇩ HANDOFF 76, 16 Sep 2026 — ***RELEASE_1.1 43 IS SETTLED: OPTION 3c IS DEAD (a virtual service account cannot adopt the broker token without `SeTcb`, which is 3a). OWNER CHOSE THE RELAY/SESSION SPLIT, AND ITS FOUNDATION IS PROVEN. THE SOCKET HANDOVER IS THE NEXT PROBE.*** ⇩⇩⇩
>
> | | |
> |---|---|
...
> | git | 43's outcome + `probe-svcimp` are on `origin/main` (`bfa7123`); this commit adds `probe-relaydrop` (the split's foundation probe) + record notes. Both probes are in `$neverShipped`; the `.exe`s stay gitignored |
...
>
> **NEXT, in order.**
> 1. ***The socket-handover probe — the crux.*** Can the bare relay PROCESS receive the accepted network socket (an inheritable handle across `CreateProcessAsUser`, then Cygwin fd adoption via `cygwin_attach_handle_to_fd`) and run TLS over it? Fold in the Low-integrity drop (proven-mechanism; needs a Low output subdir). `sdwind.c:400` measured that naive socket-passing to a Cygwin child fails, so this decides the architecture. Extend `probe-relaydrop`.
> 2. ***Then the product build*** — the relay becomes a spawned bare-account process; RELEASE_1.1 43's "build" list still applies for the `SeTcb`-holding session (`sd.c` start gate, the broker + IPC + SCRAM check, `install-service.ps1`).
>
