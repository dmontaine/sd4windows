...
> # ⇩⇩⇩ HANDOFF 78, 16 Sep 2026 — ***RELEASE_1.1 43: ITERATION 5 IS `ANSWERED (FULL)`, OWNER-ELEVATED. THE PROBING PHASE IS OVER — EVERY PIECE OF THE LINUX PER-CONNECTION SHAPE IS NOW MEASURED ON WINDOWS. WHAT IS LEFT IS THE PRODUCT BUILD, AND NONE OF IT IS DESIGNED.*** ⇩⇩⇩
>
> | | |
> |---|---|
> | install | **16 Sep 11:53:16**, unchanged. **No product source changed this session** — a probe banner string, the record, and rebuilt (gitignored) `.exe`s only |
...
>
> ***THE KEPT OBJECTION IS NOW THE LOAD-BEARING ONE, AND IT HAS A CHEAP UNELEVATED TEST NOBODY HAS RUN.*** A native relay is a **second relay source** beside Linux's `sd_tlssrv.c`, so a relay security fix would have to land twice — that is the standing cost of the direction, and it was never resolved by measurement, only accepted. HANDOFF 77 named one untried alternative and **rejected it unmeasured**: a separately-pathed second copy of `msys-2.0.dll` for the relay, which would keep **one** relay source. *It would be worth measuring first, because the error that forced native names a path-derived key* — `\BaseNamedObjects\msys-2.0S5-dd50a72ab4668b33` — *so a copy of the DLL at a different path would be expected to get its own object directory and start at Low beside `sd`.* **Falsified if** an MSYS2 program built against a relocated `msys-2.0.dll` still dies `0xC0000142` at Low while a Medium process of the shipped runtime is alive, or if it starts but the two runtimes interfere. The test is `probe-lowmsys.c`'s shape with a copied DLL, **unelevated and free**; it costs no token and no cycle. *The counter-cost if it passes is real and is the owner's call: two MSYS2 runtimes shipped, against two relay sources maintained.*
>
