...
> # ⇩⇩⇩ HANDOFF 76, 16 Sep 2026 — ***RELEASE_1.1 43 IS SETTLED: OPTION 3c IS DEAD (a virtual service account cannot adopt the broker token without `SeTcb`, which is 3a). OWNER CHOSE THE RELAY/SESSION SPLIT. THE PROBE IS COMMITTED; THE SPLIT IS NOT BUILT.*** ⇩⇩⇩
>
> | | |
> |---|---|
...
> | git | base `0224ddf` (HANDOFF 75, already on `origin/main`); **this commit adds the probe + 43's outcome and is 1 ahead of `origin/main`, UNPUSHED** (owner said commit, not push). `gplbld/probe-svcimp.c`/`.ps1` are now **committed and in `$neverShipped`**; the `.exe` stays gitignored |
...
> | ***THE DIRECTION — OWNER, 16 Sep*** | **Split the relay from the session.** The review finding is only that the RELAY parses unauthenticated bytes as LocalSystem; the relay never adopts, so it runs as a **bare unprivileged account** (the Linux `nobody` shape) and that closes the finding. The authenticated **session keeps `SeTcb`** (LocalSystem as today, or a dedicated account), because only it adopts. **Not built** |
...
>
> **NEXT, in order.**
> 1. ***Build the split.*** Relay → a bare unprivileged account (closes the review finding); the session stays privileged. Large — RELEASE_1.1 43's "build" list still applies for the `SeTcb`-holding-session parts (`sd.c` start gate, the broker + IPC + SCRAM check, `install-service.ps1`), minus the "low-priv session" premise that just died.
> 2. **Push** — one commit is sitting unpushed (owner said commit only this session).
>
