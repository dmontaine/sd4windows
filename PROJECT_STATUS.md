...
> | ***NEXT — THE CUTOVER PROBE, THEN PRODUCT CODE (conditional; not measured)*** | Probe the relay app-side **socketpair→pipe cutover on a post-auth signal, no byte loss, clean teardown**, before any product code — the one unmeasured piece of the parity build. *Falsified-if:* the app-side cannot switch without losing buffered bytes (fallback: the front stays a dumb plaintext byte-pump, a SYSTEM process in the path parsing nothing — weaker than parity, owner's call). **Also still owed, owner-elevated:** confirm the crux-(1) chain under `CreateProcessAsUser` as a **DIFFERENT** user, and cross-USER DACL denial (both should hold — the fd layer is token-independent and 43 ran a Cygwin child under a foreign S4U token; a kernel DACL denies other SIDs) |
