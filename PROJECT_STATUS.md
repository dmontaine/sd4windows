...
> | | |
> |---|---|
> | install | **16 Sep 22:22:37 — THE RELAY BUILD, CYCLED BY THE OWNER** (`assert-current` green in all four steps; `bin\` built 22:21:28). ***BUT THE TREE IS STALE AGAIN BY ONE FILE***: `install-service.ps1` was fixed after that cycle (row below), so **the next act is still a cycle** |
> | tokens | **`b167` SPENT, 22:24** — the PARTIAL run below. **Use `b168`** |
...
> ```
> 2. **The witness** (elevated PowerShell). `verify-relayidentity` holds one connection and reads the relay's token; the three API steps beside it prove the session still logs in through the new relay. **Falsified-if**: the relay is not `<machine>\sdrelay`, not Low (4096), not 0 privileges, its parent is not `sd.exe` as SYSTEM, or the handshake never completes — the SD error log then carries the relay's own refusal text (`SD API TLS: …`), which is where to read first.
> ```powershell
> powershell -ExecutionPolicy Bypass -File C:\Users\Don\SDCoreProject\sd4windows\sdb_ai\sd64\gplbld\VerifyInstall2.ps1 -Run b168 -Only verify-relayidentity,verify-apiport,verify-scramlogin,verify-apiidentity
> ```
...
>
> ***WHAT WOULD MOST PLAUSIBLY GO WRONG ON THE INSTALL, SO IT IS RECOGNISED RATHER THAN RE-DERIVED (conditional; none observed):*** (i) `CreateProcessAsUser` refusing with 1314 — the session's token lacks `SeAssignPrimaryToken` (LocalSystem holds it; a probe of the service's `sd -n` token would settle it); (ii) the relay dying `0xC0000142` — a UCRT DLL not reachable from the account's environment (the relay's imports are only KERNEL32/ADVAPI32/CRYPT32/USER32/WS2_32 and `api-ms-win-crt-*`, all system; `CreateEnvironmentBlock` supplies `SystemRoot`); `sd_tlssrv.c` maps that code to *"could not initialise (0xC0000142 …)"* in syslog; (iii) `LsaLogonUser` refusing `sdrelay` — `STATUS_LOGON_TYPE_NOT_GRANTED` would mean a deny right too many (only four are set, network is NOT among them), `STATUS_ACCOUNT_DISABLED` would mean something disabled it; (iv) the relay unable to open `usr\bin\sdtlsrelay.exe` — Program Files grants `Users`, and a local account is in `Users` through `Authenticated Users`; a `secure-*` script tightening `{app}` would break that and none does today.
>
