...
> | ***PRODUCT BUILD STARTED — SLICE 1 DONE (`win32session.c`, syntax-clean)*** | `gplsrc/win32session.c`+`.h`: `win32_session_spawn(user, pipe, &proc, &pid)` mints the user's token and `CreateProcessAsUser`s `sd -N -A <pipe>` AS the user, no privilege strip / no integrity drop (the user's own session, like Linux setuid), inherits nothing (the session opens the pipe itself). Makefile needs no change (`gplsrc/*.c` wildcard; `-luserenv`/`-lsecur32` already linked). **Not yet called.** The full file-by-file spec is in **HISTORY.md 17 Sep** — the one new mechanism beyond the probes is a front↔relay **control channel** (a second socketpair) carrying the pipe name + user SID + session PID at cutover |
