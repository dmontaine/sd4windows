...
> # ⇩⇩⇩ HANDOFF 80, 16 Sep 2026 — ***RELEASE_1.1 43 IS DONE AND WITNESSED: THE TLS RELAY RUNS AS `sdrelay`, LOW, 0 PRIVILEGES, ONE PER CONNECTION, ON THE INSTALL (`verify-relayidentity` 15/15, `b169`). THREE CYCLES TO GET THERE, EACH FAILURE CAUGHT BY AN INSTRUMENT AND REPRODUCED UNELEVATED. OWED: THE FULL SUITE.*** ⇩⇩⇩
>
> | | |
> |---|---|
...
> | ***WHAT IS OWED*** | **1. The full suite, both halves** (CLAUDE.md: before a hand-off) — every API step crosses the relay now, and `verify-apiname`, `verify-apiadmin`, `verify-apiremote`, `verify-tierapi` have not seen it. **2. (f), the load question**: `LsaLogonUser` once per connection under concurrency — unmeasured; `probe-relayscale` measured process start-up, never the mint. **3. RELEASE_1.1 53** (S, owner's call), unchanged: `sd`'s own MSYS2 `shared.5` is writable by any local user at Medium. |
...
>
> **NEXT — THE FULL SUITE, both halves, again.** The tree is cycled and current and the two fixed scripts ship nowhere, so no cycle is owed; from an **ordinary unelevated** prompt signed in AFTER the 22:55 cycle (the runner elevates its second half itself):
> ```powershell
> powershell -ExecutionPolicy Bypass -File C:\Users\Don\SDCoreProject\sd4windows\sdb_ai\sd64\gplbld\VerifyInstall1.ps1 -ThenElevated -Run b173
> ```
> *The first half's last step is now `verify-elevdoor` (unelevated half); the second half opens with its elevated half. Two full-suite attempts in a row have each found one more verifier that RELEASE_1.1 45 silently invalidated — if a third appears, read its transcript for the door's own refusal text before anything else.*
