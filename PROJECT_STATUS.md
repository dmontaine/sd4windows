...
>
> ### ⇩ THE ONE THING OWED IS A SUITE RUN ⇩
>
> **Two cycles were spent after `b66`, so the suite has not run against this
> install.** ORDINARY, UNELEVATED PowerShell — **not** elevated (§4.0.1):
>
> ```
> C:\Users\dmont\Projects\sd4windows\sdb_ai\sd64\gplbld\VerifyInstall1.ps1 -ThenElevated -Run b67 -ContinueOnFailure
> ```
>
> ***EXPECT 14 OF 14 UNELEVATED, NOT 13*** — `verify-txn.ps1` was added to the
> unelevated list, so the count moved. **19 of 19 elevated.** `b54`–`b66` are
> spent.
>
...
>
> ### ⇩ THE THREE BLOCKERS LEFT, AND WHAT EACH IS WAITING ON ⇩
>
> ***56, 57 AND 58 CLOSED ON 29 Aug 2026 AND ARE OFF THIS LIST.*** 56's model is
> built, cycled and proved by `-Run b66`; 57 likewise, its "installed and unrun"
> having been a testing gap rather than outstanding work; 58's `Administrator`
> set is written. **56's one remainder was re-filed as 62 rather than dropped.**
>
> | | waiting on |
> |---|---|
...
> | **61** | one measurement: **which of `newvoc` / `voc_template` feeds SDSYS's VOC.** `verify-lcnames.ps1:772` and the live reading disagree. ***Do not paste the `F` in until that is answered*** |
> | **62** | **56's remainder, re-filed rather than dropped**: whether `elevate('START')` still admits somebody who is not an administrator. It tests nobody's identity, so an administrator's password was enough and the trail named the wrong person — but that was measured **before** clause 2 was reversed, and ***the reversal plausibly closes it and nobody has looked*** |
>
> ***ENTRY 2 IS OPEN ON A PREMISE THAT NO LONGER HOLDS — READ IT BEFORE WORKING
> IT.*** It was re-opened as downstream of 56 and says *"56 abolishes the
> administrator account this attached to"*; **clause 2's reversal gave that
> account back**, so the stated reason for re-opening is gone. It is left open
> rather than quietly closed, because whether the `os.users` half still matters
> under the model that actually shipped has not been measured. **58 no longer
> blocks 34 and 55, but they still collide — do them together.**
>
> ***NOTHING NEEDS A DECISION FROM THE OWNER TO PROCEED*** except 61's, which is
> a measurement first and a ruling second.
>
