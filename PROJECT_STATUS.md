...
>
> ### ⇩ A CYCLE IS OWED FIRST, THEN THE SUITE RUN. IN THAT ORDER. ⇩
>
> ***`assert-current` IS RED AND THAT IS DELIBERATE, NOT A FAULT TO INVESTIGATE.***
> The 82nd session changed **twelve `sdsys/voc_template` records** for
> PRE_RELEASE 63, so check B sees source newer than the 18:55:20 install and
> every verifier refuses until a cycle runs. **Nothing is broken; the install is
> simply older than the tree.**
>
> **Step 1 — the cycle. ELEVATED PowerShell:**
>
> ```
> C:\Users\dmont\Projects\sd4windows\sdb_ai\sd64\gplbld\cycle.ps1
> ```
>
> **Step 2 — the suite. ORDINARY, UNELEVATED PowerShell — *not* elevated
> (§4.0.1):**
>
> ```
...
> spent.
>
> **Step 3 — what closes 63, and nothing else does. ELEVATED PowerShell**, which
> lands in SDSYS; type `listf` at the prompt:
>
> ```
> & 'C:\Program Files\SD\usr\bin\sd.exe'
> ```
>
> ***THE TEN ROWS THAT PRINTED A BARE `F` MUST NOW READ `File - …`*** — `$hold`,
> `accounts`, `bp`, `bp.out`, `gpl.bp`, `gpl.bp.out`, `messages`, `newvoc`,
> `os.users`, `qfile`. **`$MAP` must still read `DH`**, which is the control:
> it was the row that started 61 and it was never broken.
>
