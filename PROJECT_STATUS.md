...
> | — | ***OWNER'S CALL, §5.9***: `ApplyAllowGroups` runs on any machine (limitssh is ticked by default, no `SshWasAbsent` gate — [sd.iss:210](sdb_ai/sd64/gplbld/sd.iss:210), :965). The **existing-Allow/Deny refusal** in `allow-ssh-groups.ps1` catches machines with any configured policy, but NOT a bare stock sshd. **Three shapes:** (A) add `Flags: unchecked` at `sd.iss:210` — safer default, admin must tick to confine; reverts the 21 Aug flip. (B) restore the `SshWasAbsent` gate at `sd.iss:965` — default-ticked only fires when SD installed sshd itself; matches ApplySshFirewall. (C) no change; the task's own description names scp/sftp as the sharp edges and that is the warning. Stale header at `allow-ssh-groups.ps1:31` was fixed today; the shapes are now the decision | decision not started |
