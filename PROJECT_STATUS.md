...
> | ***NEXT — PRODUCT CODE + A CYCLE (no more probes); needs the owner's go, it is a large change to the API auth path*** | **(1)** login tail (`win32s4u.c` `AssumeUserIdentity` callers in APISRVR / the `sd_tls` path): replace the in-place `seteuid` adopt with — after SCRAM reveals the user — mint the token (`win32_s4u_logon`, already lifted), create the SID-ACL'd pipe + PID bind (crux 1), `CreateProcessAsUser` `sd` AS the user with the pipe name + the 32-byte tls-exporter binding, and signal the relay to cut over (`win32relay.c`/`sd_tlssrv.c`). **(2)** the session `sd -n` opens the pipe as its fds 0/1 and does NOT run SCRAM (handed the verified username; it IS the user, so the `K_ASSUME_USER` equivalent is gone). **(3)** graceful pipe close (`FlushFileBuffers`, not `DisconnectNamedPipe` — the ECOMM lesson). **(4)** verify: `verify-apiidentity` should now show the session's REAL identity is the user, no `SeTcb`, no SYSTEM underneath (turn `verify-relayidentity`'s parent control onto the session); a cycle proves it, and the two owner-elevated crux-(1) confirmations (as a DIFFERENT user; cross-USER DACL denial) fold into it. Full plan: HISTORY.md 17 Sep |
