...
> 1a. ***OWNER, 16 Sep 2026: "whichever is closest to the Linux model."*** Linux (`gplsrc/sd_tlssrv.c`) is a relay **per connection**, holding the network descriptor from birth, dropping to `nobody` before parsing, talking to `sd` over a **private `socketpair`**. B's shared listener and loopback port match neither half. So `probe-relaydrop` **iteration 3** (built, loads, **UNRUN** — needs the owner's elevated shell) tests the per-connection shape: the spawned bare child uses the inherited SOCKET through **native Winsock** (imports verified from `WS2_32.dll`; both dead mechanisms failed in Cygwin's fd layer, not the socket), and relays what it read over an **inherited anonymous pipe** the parent reads as a **Cygwin fd**. *If* both legs work, B should not be needed; *if* the native socket leg fails, B returns. Not covered: recovering the SOCKET from `sd`'s Cygwin fd (`_get_osfhandle`) before handing it over.
