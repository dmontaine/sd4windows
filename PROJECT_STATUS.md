...
> | ***NEXT (conditional; not yet measured)*** | Two owner-elevated confirmations of (a)'s handover, then crux (2). **(i)** the whole chain under `CreateProcessAsUser` as a **DIFFERENT** user (this run was same-user; the fd layer is token-independent, so it should hold — 43 already ran a Cygwin child under a foreign S4U token); **(ii)** cross-USER DACL denial (a kernel guarantee, but exercise it). Then **crux (2)** — the larger design half — moving the SCRAM parse to an **unprivileged** front so nothing is SYSTEM even pre-auth (the OpenSSH-privsep / Linux-`nobody` shape), and the product build where the front spawns the session as the user and hands it the pipe name + the 32-byte tls-exporter binding. *Falsified-if (i):* the as-a-different-user run cannot open or poll the pipe |
> | ***DO NOT RE-SPEND*** | the socketpair-inheritance handover — measured dead, both directions (`EFAULT`; 43's `EINVAL`/`EBADF`). The reconnect **channel** is settled (named pipe, PASS); the residual is a same-user local-DoS (grabbing the single pipe instance), which is not a privilege gain and is accepted unless the owner says otherwise |
>
