...
> # ⇩⇩⇩ HANDOFF 75, 16 Sep 2026 — ***43's PROBE IS BUILT AND HAS RUN FOUR TIMES. ITS LAST RUN PRINTED "FALSIFIED" AND THAT VERDICT IS FALSE — DO NOT FILE IT. THE PROBE IS UNCOMMITTED.*** ⇩⇩⇩
>
> | | |
> |---|---|
> | install | **16 Sep 11:53:16**, `cycle-20260916-115222.log`, `CYCLE COMPLETE`. Unchanged since; nothing in this session touched product source. ***AN UNELEVATED `assert-current` EXITS 2 HERE AND IT IS NOT A FAULT*** — the cycle recreated `sdusers` with a new SID, so any shell older than 11:53 is denied `sdsys`; the script names the cure (sign out, PRE_RELEASE 182) |
> | tokens | `b167` **still unspent** |
> | git | `38cc04f`, **1 commit ahead of `origin/main`, unpushed**. ***UNCOMMITTED AND UNTRACKED: `gplbld/probe-svcimp.c` and `gplbld/probe-svcimp.ps1`*** (plus the gitignored `.exe`). They are RELEASE_1.1 43's gate and nothing in the tree references them yet — **not in `$neverShipped`, not in CLAUDE.md's free tier, no entry text written** |
> | ***43 — WHAT IS MEASURED AND IS SAFE TO KEEP*** | Owner chose **option 1**: a throwaway `sdprobe` service, so the real SD service is never touched. Run elevated, it got all the way through the handoff. **`NT SERVICE\sdprobe` holds `SeImpersonatePrivilege` — YES** (the entry called this *"expected, not measured"*). **It does NOT hold `SeTcbPrivilege`** — which is why the broker exists. **The LocalSystem broker S4U-minted a token for `ace\Don`**, and **`DuplicateHandle` carried it into the worker**, which received it and read it back as `ace\Don`. Those five facts stand |
> | ***AND THE VERDICT THAT DOES NOT — READ THIS BEFORE BELIEVING THE TRANSCRIPT*** | The 4th run printed ***`FALSIFIED: the adopted child wrote a file owned by NT SERVICE\sdprobe, not by Don`***. **It is not a measurement of 43.** Three lines in the same log say the run never reached the state it claims to test: `after impersonate : thread token NONE` on an `ImpersonateLoggedOnUser` that RETURNED SUCCESS; `seteuid(197610) failed - Operation not permitted`; and `uid -1 euid -1`. ***probe-impfork's Q4 is explicit that the bare `CW_SET_EXTERNAL_TOKEN` changes nothing and `seteuid` is what makes the runtime adopt*** — so an "adopted child" measured after a FAILED `seteuid` adopted nothing. **The plain-fork CONTROL passed trivially for the same reason, which is exactly what made the whole thing look consistent** |
> | ***the two gates added after that, NOT YET RUN*** | The probe now **refuses (exit 2) instead of scoring** at both points: (1) if neither view of the thread token names the target after impersonating — it now reports `OpenAsSelf` **both ways** with the error number, because `OpenAsSelf TRUE` access-checks against the PROCESS token and may fail to open a token that is genuinely attached; (2) if `seteuid` fails. It also prints `uid`/`euid` before anything and whether `getpwnam` resolves. **Built clean (`-Wall -Wformat=2`); the 5th run has not happened** |
> | ***the live hypothesis, conditional*** | `getuid()` reading **-1** before any switch would mean the MSYS2 runtime has no user identity for a virtual service account at all. **If that holds, `seteuid` can never run there, and the finding is about the RUNTIME'S USER MAPPING rather than about option 3c** — different problem, different remedies (a passwd mapping for the service SID, or a different adoption route). *Not established; the next run's `uid`/`getpwnam` lines decide it.* |
> | ***three bugs of mine, all fixed, worth knowing*** | (a) a virtual service account's **name does not resolve until its service exists**, so `icacls` must run AFTER `sc create` — ***and having the right SID is not the same as having a principal***: `sc showsid` derives it happily with no service, and `icacls *S-1-5-80-…` still refused. (b) ***`2>&1` on a native exe***: PowerShell 5.1 wraps stderr in a `NativeCommandError` and trips `$ErrorActionPreference='Stop'` on a program that returned 0. (c) ***`fscanf("%lu", (unsigned long*)&dword)`*** — under this runtime `unsigned long` is **8** bytes and `DWORD` is 4, so it wrote past the variable into the next one on the stack; the path printed fine, a write test through it SUCCEEDED, then the same variable came back EMPTY. **The CAST is what hid it** — `-Wformat=2` would have caught the bare mismatch |
> | ***37 — INVESTIGATED, THEN REMOVED BY THE OWNER*** | A cause was measured and committed (`921212b`), and **the owner then asked for all four files restored to their previous state**, which was done by dropping that commit. `921212b` is recoverable from the reflog. ***Do not re-add any of it without asking him*** — the removal was deliberate and its reason was not given. The measurements themselves are in this session's transcript |
>
> **NEXT, in order.**
> 1. ***Decide whether `probe-svcimp.*` is kept at all.*** It is uncommitted on purpose: the owner removed the previous probe, and this one was never shown to him committed. If it stays it needs `$neverShipped` (`probe-svcimp.c`, `probe-svcimp.exe`), and 43's row needs the five measured facts above.
> 2. **Re-run it, elevated**, and read the new `uid`/`getpwnam`/two-way-token lines: `powershell -ExecutionPolicy Bypass -File C:\Users\Don\SDCoreProject\sd4windows\sdb_ai\sd64\gplbld\probe-svcimp.ps1 -Account Don`. It creates and removes a `sdprobe` service, a `sdprobebroker` task and `C:\ProgramData\sdprobe-run` on every exit path; `-Keep` leaves them. **`sc start` reporting 1053 is by design.**
> 3. **Push** — one commit is sitting unpushed.
>
