...
> | ***MEASURED — AND MY FIRST EXPLANATION OF IT WAS WRONG*** | The obvious reading was *"the key is path-derived"*. `probe-relockey.ps1` put copies at **three** different shapes — a plain directory, a deeper one, and a real `<root>\usr\bin` layout — and held all three alive beside the shipped runtime: **3 copies produced 1 relocated key**, `dd50a72ab4668b33`, not three. **FALSIFIED.** The key separates *registered* installs; **every unregistered copy on the machine shares one namespace** |
>
...
>
> ***WHAT THAT LEAVES, AND IT IS A TRADE RATHER THAN AN ANSWER (the three options, stated plainly):*** **(1) native relay at Low** — the shipped-source cost, two relay sources maintained forever, and it is what HANDOFF 77 chose. **(2) relocated MSYS2 relay at Low** — one relay source, `sd_tlssrv.c` shared with Linux, *but* the relay's runtime namespace is shared with **any other software on the machine shipping an unregistered `msys-2.0.dll`**, which is the **RELEASE_1.1 53** class of exposure moved rather than removed, and 53 is the finding that made the Low drop mandatory in the first place. **(3) a properly *registered* second install** — would be expected to get its own key like `C:\msys64` does, ***and it is UNMEASURED***: what makes a layout "registered" was not established, the `<root>\usr\bin` shape alone did not do it. **Falsified if** a registered second install still shares a key, or if registering one has install-time cost the product will not carry. ***ONE HYPOTHESIS IS ALREADY DEAD, MEASURED THE SAME SESSION — IT IS NOT THE REGISTRY.*** `HKLM`/`HKCU` `\SOFTWARE\Cygwin` and `…\Cygwin\Installations` are **all four absent on this box**, so `C:\msys64`'s distinct key cannot come from a registration entry; whatever makes it distinct is **inside its own tree** (`/etc` is the obvious next suspect, and the three falsified copies had none). *That lowers the odds (3) is cheap, and it is why (3) is written as the thing to measure rather than the thing to build.* *If (3) held it would dominate — one relay source and a private namespace — so it is the cheap thing to measure before the owner is asked to choose between (1) and (2).*
>
