...
> | | |
> |---|---|
> | install | 15 Sep 00:30:17 (cycle, no bootstrap "not found"); `assert-current` still matches — both edited scripts are in `$neverShipped`. ***DIRTY SINCE 01:20:55***: `verify-deadlock` run 3 killed a session; user slot 66 and its RU lock are still live in shared memory and `sdsys/zzdlfile` could not be deleted. **Cycle before measuring anything** |
...
>
> **Owed:** a **cycle, elevated** — run 3 killed a session and left a dead lock and `sdsys/zzdlfile` behind; nothing is measurable until then. **Optional, before the cycle and only if the owner wants 37 narrowed** (conditional, his call): `C:\Program Files\SD\usr\bin\sd.exe -cleanup` from an elevated prompt, then a fresh `sd` session running `LIST.READU` — if the lock is gone, the daemon's detection (`kill(pid,0)`, `sdwind.c:262`) is the suspect; if it stays, `sd -cleanup` itself is. Then 5's remaining half (the real W1.0-0 upgrade, owner: later) and 18 (at W1.1-0 assembly). *Item 7 may have no route to a witness by killing while 37 holds*: on this one run the owner was not unmapped while its lock stood, and `(gone)` needs the opposite order (slot gone, lock still there). One run, one window; do not treat it as the rule until a second run or the 37 probe says so.
> ```powershell
