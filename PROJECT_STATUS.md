...
> | | |
> |---|---|
> | install | **17 Sep 01:26:28 — the cycle carrying RELEASE_1.1 37's fix** (`sdwind.exe` rebuilt, `assert-current` green). `verify-deadlock` ran on it at 01:27 and **killed one session, which the daemon then reaped** — its last line asks for a cycle before the next measurement; no product source has changed since |
...
>
> **NEXT.** `verify-deadlock` killed a session, and its last line asks for a cycle before anything else is measured — **the tree is current, but the next suite measurement wants a fresh install first** (**elevated PowerShell**):
> ```powershell
> powershell -ExecutionPolicy Bypass -File C:\Users\Don\SDCoreProject\sd4windows\sdb_ai\sd64\gplbld\cycle.ps1
> ```
> Then the open rows before the parity audit (47): **40** (upgrade progress captions — needs a real W1.0-0 → W1.1-0 upgrade to witness, the W1.0-0 installer on the box first), **53** (the writable `shared.5` — a ruling), **54** (print spooling and sendmail dead on the install — a ruling). Also owed a restart of the machine: 29 orphaned test profiles under `C:\Users` have loaded hives (PRE_RELEASE 185) and cannot be swept until one; after it, `clean-test-profiles.ps1` (elevated) or the next `VerifyInstall2` takes all 36, six of which only became sweepable at `240d582`.
>
