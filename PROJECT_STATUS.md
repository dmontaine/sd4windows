...
> | ***owed, in order*** | 1. ***DONE — the cycle ran green (install row above); `dll_closure` did copy OpenSSL beside `sd.exe`, measured.*** 2. **the live TLS witness**: `verify-apiport.ps1` (elevated) drives `remote-connect-test.exe` through `sdclilib.dll`, which now does TLS end-to-end — the natural client↔server witness — plus a packet capture on 4243 showing nothing readable, and a **Windows client against a Linux server** and vice-versa. 3. ***RELEASE_1.1 42*** — the plaintext API verifiers (`verify-scramlogin`, `verify-peerlog`, `verify-apiidentity`'s raw leg) now break against the TLS-only server and must be taught TLS (or retired to the client DLL) before the next full suite; the client-DLL verifiers are already fine. 4. uninstaller removal of `sd-tls` (ties to finding 38) |
> ```powershell
