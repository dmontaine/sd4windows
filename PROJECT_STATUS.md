...
> # ⇩⇩⇩ HANDOFF 80, 16 Sep 2026 — ***RELEASE_1.1 43: THE PRODUCT RELAY IS BUILT. `sdtlsrelay.exe` RELAYS A REAL TLS 1.3 SESSION AND HANDS sd THE CLIENT'S OWN BINDING (27/27, MUTANT-CONTROLLED). NOT YET CYCLED, NOT YET SEEN ON AN INSTALL. FIRST ACT: ONE CYCLE, THEN `verify-relayidentity`.*** ⇩⇩⇩
>
> | | |
> |---|---|
> | install | **16 Sep 22:41:04 — the relay build's SECOND cycle** (`assert-current` green). ***STALE AGAIN, BY `sdtlsrelay.c` AND `stage.py`*** (the USER32 fix, row below), so **the next act is a THIRD cycle** |
> | tokens | **`b167` and `b168` SPENT** (22:24 and 22:42, the two PARTIAL runs below). **Use `b169`** |
...
>
> **NEXT — IN THIS ORDER, EACH GATED ON THE ONE BEFORE.**
> 1. **The cycle** (elevated PowerShell). Step 0 rebuilds the C including the new `sdtlsrelay` target; `stage.py` will print `sdtlsrelay.exe is native (N imports, none msys-*)` or refuse. The install runs `install-service.ps1 -Install` hidden; **since the fix it transcribes to `C:\ProgramData\SD\install-service.log`, so read that first** — it must carry `created the relay account sdrelay` and `denied interactive, remote interactive, batch and service logon`, and must NOT carry `IS NOT IN PLACE`. `verify-relayidentity`'s step 1 checks the account exists, is enabled and is in no group, and refuses everything else if it is absent. *(The first cycle already proved `stage.py`'s `NATIVE_ONLY` line and the relay's build: `bin\` was current and the tree matched source.)*
> ```powershell
> powershell -ExecutionPolicy Bypass -File C:\Users\Don\SDCoreProject\sd4windows\sdb_ai\sd64\gplbld\cycle.ps1
> ```
> 2. **The witness** (elevated PowerShell). `verify-relayidentity` holds one connection and reads the relay's token; the three API steps beside it prove the session still logs in through the new relay. **Falsified-if**: the relay is not `<machine>\sdrelay`, not Low (4096), not 0 privileges, its parent is not `sd.exe` as SYSTEM, or the handshake never completes — the **Application event log (`sd_Log`)** then carries the relay's own refusal text (`SD API TLS: …`), which is where to read first (row above).
> ```powershell
> powershell -ExecutionPolicy Bypass -File C:\Users\Don\SDCoreProject\sd4windows\sdb_ai\sd64\gplbld\VerifyInstall2.ps1 -Run b169 -Only verify-relayidentity,verify-apiport,verify-scramlogin,verify-apiidentity
> ```
> 3. **The full suite before hand-off**, both halves, as CLAUDE.md requires — `verify-apiname`, `verify-apiadmin`, `verify-apiremote`, `verify-tierapi` all cross the relay now.
>
