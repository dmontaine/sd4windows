...
>
> # ⇩ TWO MORE RULINGS, AND A FINDING THAT MAKES ONE OF THEM URGENT. ⇩
>
> ***OWNER, 29 Aug 2026: "Two tiers" (56) and "Proceed, and print what it
> voided" (57).*** Both are **traced and recorded, NOT YET BUILT** — that is
> the next session's first job, and everything needed is derived below so
> nobody re-derives it.
>
> ***THE FINDING: `CREATE.ACCOUNT … ADMINISTRATOR` MAKES THE USER A WINDOWS
> ADMINISTRATOR.*** `CREATEA:813` → `make.admin` →
> `os_group("ADDMEM", "S-1-5-32-544", …)`, the **built-in Administrators
> group**. Under 56 that single command yields a Windows administrator who is
> elevated at login into SDSYS and so **never enters the account just made for
> them**, plus `sdssh`/`sdapi` membership 56 says they cannot use. ***It is a
> contradiction with 56, and it is an SD verb that grants Windows
> administrator.*** Dropping the tier removes that path.
>
> ***THE FOOTPRINT IS 15 TIER LITERALS IN FOUR FILES*** — `CREATEA` (6),
...
> they ask whether the **person** is a Windows administrator, which is what 56
> keeps.
>
> ***AND IT MUST STAY READABLE WHILE IT STOPS BEING OFFERED — MEASURED.***
> `C:\ProgramData\SD\sdsys\accounts\don` holds **`ADMINISTRATOR` in field 5**
> right now. The data tree is never upgraded (§6), so removing it from
> `TIERGATE:132`'s `tg.tiers` would make `tier_allows` answer *"no usable
> tier"* for an existing account and break its `logto`. **Stop OFFERING it in
> `CREATEA`/`MODIFYA`; keep RECOGNISING it in `TIERGATE`.**
>
> **57 is narrower than it reads**: `LOGIN` is not a `tier_allows` caller, so
> the account's own owner is never stranded and a single-member account cannot
> strand anything. **Both changes touch `MODIFYA` — do them in ONE cycle.**
>
