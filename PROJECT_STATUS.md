...
> | ***WHAT IS UNWITNESSED*** | The `verify-sdsysgate` wording fix (`518afb7`) has not run since it was made: one cheap step from an **elevated** prompt witnesses it — `VerifyInstall2.ps1 -Run b174 -Only verify-sdsysgate` (below). And (f), `LsaLogonUser` per connection under concurrency |
...
> | ***WHAT IS OWED*** | ~~**1. The full suite, both halves**~~ — **done, `b173`, row below.** **2. (f), the load question**: `LsaLogonUser` once per connection under concurrency — unmeasured; `probe-relayscale` measured process start-up, never the mint. **3. RELEASE_1.1 53** (S, owner's call), unchanged: `sd`'s own MSYS2 `shared.5` is writable by any local user at Medium. |
...
>
> **NEXT.** Nothing product-side is owed for 43. The one loose end is the `verify-sdsysgate` wording fix, made after its step ran on `b173` and so unwitnessed; one step from an **elevated** PowerShell (`-Run` still required, it derives the prefix; tokens `b167`–`b173` are spent):
> ```powershell
> powershell -ExecutionPolicy Bypass -File C:\Users\Don\SDCoreProject\sd4windows\sdb_ai\sd64\gplbld\VerifyInstall2.ps1 -Run b174 -Only verify-sdsysgate
> ```
> *Then the open list is (f) — `LsaLogonUser` per connection under concurrency, unmeasured, the same work whichever way the relay had gone — and RELEASE_1.1 53. Three suite attempts each found one more verifier that RELEASE_1.1 45 had silently invalidated (`lcnames`, `sdsyswrite`, `sdsysgate`); those were the three that either issued `LOGTO SDSYS` from an unelevated start or anchored on the pre-45 audit reason, and no others do.*
