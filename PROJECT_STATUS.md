...
> | ***`b174` 00:34 — `verify-sdsysgate` 11/11 DECISIVE, WITNESSED*** | The non-administrator's audit line read live: `LOGTO REFUSED account=SDSYS reason=session did not start elevated`, the pre-45 reason absent, the elevate call never reached, nothing granted. **Every red row of `b170`–`b173` is now closed and witnessed.** Tokens `b167`–`b174` spent; **use `b175`**. Unmeasured: (f), `LsaLogonUser` per connection under concurrency |
