...
>
> **Owed:** nothing is cycle-owed. Open: **37** (owner's call on severity; the cycle wiped run 3's state, so narrowing it means staging a dead lock again — `verify-deadlock` does that — then `sd -cleanup` elevated and `LIST.READU` in a fresh session: lock gone → the daemon's detection, `kill(pid,0)` at `sdwind.c:262`, is the suspect; lock stays → `sd -cleanup` itself is); **7** (unwitnessed, route depends on 37); **5**'s remaining half (the real W1.0-0 → current upgrade, owner: later, before release); **18** (at W1.1-0 assembly). *Item 7 may have no route to a witness by killing while 37 holds*: on this one run the owner was not unmapped while its lock stood, and `(gone)` needs the opposite order (slot gone, lock still there). One run, one window; do not treat it as the rule until a second run or the 37 probe says so.
> ```powershell
