...
> | ***NEXT — THE REMAINING SLICES, THEN A CYCLE (spec: HISTORY.md 17 Sep)*** | **(2)** `sd.c` `-A` sub-mode of `CN_SOCKET` + a `-A` session opens the pipe as fds 0/1 and skips SCRAM/relay-spawn (it IS the user). **(3)** `win32relay.c`/`sd_tls.h` control channel; **(4)** `sdtlsrelay.c` becomes the pipe SERVER (SID-ACL + PID check) and cuts over on the control message (probe-relaycutover). **(5)** `op_kernel.c` new `K$HANDOFF` key. **(6)** APISRVR `vb.scram.final`: send `v=ssig`, then `K$HANDOFF`, then the front EXITS (the spawned session runs the BASIC). **(7)** `verify-apiidentity` asserts the session's REAL token is the user, no `SeTcb`; a cycle proves it, and the two owner-elevated crux-(1) confirmations fold in. It is a large, interdependent, security-critical change — build in compile-checked slices, cycle at the end |
