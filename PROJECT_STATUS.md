...
***THE FULL-SCREEN EDITOR IS BACK AS `edit`, AND IT IS UNCOMPILED SOURCE UNTIL SOMEBODY RUNS A CYCLE.*** Owner, 26 Aug 2026, reversing the 17 Aug removal of `MICRO`. `sdsys/gpl.bp/EDIT` is `MICRO` ported with its three defects fixed (§UPSTREAM_FIXES #16); `MICRO` is deleted; `newvoc/edit` and `voc_template/edit` point at `$EDIT` instead of `$ED`. **`edit` was already in `TIER.OMIT.STANDARD`, so the tier gate needed no change** — programmer and administrator have it, standard does not. `gplbld/install-edit.ps1` is new and shipped, and `sd.iss` runs it unconditionally. ***NOTHING HAS COMPILED IT: `BASIC gpl.bp *` runs inside `SECOND.COMPILE` during a bootstrap, so `cycle.ps1 -SkipInstall` is the first thing that will say whether it parses.***
