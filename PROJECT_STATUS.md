...
> | | |
> |---|---|
> | install | **16 Sep 22:55 — the relay build's THIRD cycle**, `assert-current` green in all five steps; **no product source has changed since** |
...
>
> **NEXT.** Nothing is owed for 43 — the full suite is green in both halves, the one verifier row it found is witnessed (`b174`), and (f) is measured. No command is pending. *What is open near this work is RELEASE_1.1 53 alone (S, owner's call). Three suite attempts each found one more verifier that RELEASE_1.1 45 had silently invalidated (`lcnames`, `sdsyswrite`, `sdsysgate`); those were the three that either issued `LOGTO SDSYS` from an unelevated start or anchored on the pre-45 audit reason, and no others do.*
