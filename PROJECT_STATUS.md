...
> | tokens | **`b167`, `b168`, `b169` SPENT** (22:24, 22:42, 22:56). **Use `b170`** |
> | git | all committed and pushed on `origin/main` |
> | free tier | **43 green + `test-sysmsg-units` exit 2** (this shell predates the install, PRE_RELEASE 182). **Forty-four now**: `test-tlsrelay-units.py` and `test-installservice-units` joined |
...
>
> **NEXT — THE FULL SUITE, both halves.** The tree is cycled and current, so no cycle is owed; from an **ordinary unelevated** prompt (the runner elevates its second half itself):
> ```powershell
> powershell -ExecutionPolicy Bypass -File C:\Users\Don\SDCoreProject\sd4windows\sdb_ai\sd64\gplbld\VerifyInstall1.ps1 -ThenElevated -Run b170
> ```
