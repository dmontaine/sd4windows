...
>
> # ⇩ START HERE. ONE MEASUREMENT UNBLOCKS EVERYTHING BELOW. ⇩
>
> ***NOTHING IS IN FLIGHT. NOTHING IS HALF-BUILT.*** Session 80 ended out of
> credits with three rulings taken and traced and **no product code touched**.
> `assert-current` exit 0, units 51/0, `test-fixlist-units` 202/0, no cycle
> spent.
>
> ***STEP 1 — MEASURE `IsAdmin()` FOR AN UNELEVATED ADMINISTRATOR. EVERYTHING
> ELSE WAITS ON IT AND IT COSTS NO CYCLE.*** The restored model needs `LOGIN`
> to tell *"this session is already elevated"* from *"this person is an
> administrator"*, and **no existing key answers the first**:
> `K_ADMINISTRATOR` is a settable `USR_ADMIN` flag (`op_kernel.c:395`),
> `K_OS_ADMINISTRATOR` is `IsAdmin() && connection_type != CN_SOCKET`
> (`op_kernel.c:456`). **Do not assume `IsAdmin()` is False unelevated** — this
> file records it answering TRUE for every API session until the `CN_SOCKET`
> guard was added. A tiny BASIC probe run twice, once from an ordinary prompt
> and once from an elevated one, settles it.
>
> ***STEP 2 — THEN BUILD, IN ONE CYCLE:*** `LOGIN`'s `:398` exemption removal
> and `:513` branch (56 clause 2, below), and 57's promotion report. **Both
> touch `MODIFYA`/`LOGIN`; do not spend two cycles.**
>
