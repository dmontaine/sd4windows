...
>
> **NEXT, in order.** 1. ***Build 38's config separability in `sd.iss`*** — split `sd.conf` out of the database question into its own opt-in, both still defaulting to keep, each naming what it destroys, `/VERYSILENT` deleting neither. 2. **Decide 50 and 51** — both are filed with a fix direction and neither is ruled; 50 is in the same uninstaller code 38 touches, so it is cheap to take together. 3. ***THEN ONE CYCLE*** — `cycle.ps1` restores a clean current install **and** ships whatever of 38/50/51 has landed, which is the owner's *"one cycle serves both"*. **Do not cycle before 1–2 unless the tree is needed clean sooner**, since a cycle is the thing that makes 38's witness possible (an interactive uninstall offering both choices separately, plus a `/VERYSILENT` that keeps both).
>
